import faiss, matplotlib.pyplot as plt, numpy as np, mindspore
from PIL import Image


#######################
def evaluate(LOG, metric_computer, dataloader, dataset, model, opt, evaltypes,  # device,
    ...
    """
    # compute metrics
    computed_metrics, extra_infos = metric_computer.compute_standard(opt, model, dataloader, dataset, evaltypes)

    # split metrics by value type: numeric and histogram
    numeric_metrics = {}
    histogr_metrics = {}
    ...
    # get desc str for numeric metrics
    full_result_str = ''
    ...
    for evaltype in numeric_metrics.keys():
        for eval_metric in numeric_metrics[evaltype].keys():
            ...
            # transferred parameters: segment, content, group
            # e.g. R@1, [...], discriminative_R
...
def set_checkpoint(model, opt, progress_saver, savepath, aux=None):
    ...
    # mindspore.save_checkpoint({'state_dict': model.parameters_dict(), 'opt': save_opt, 'progress': progress_saver, 'aux': aux}, savepath)
    mindspore.save_checkpoint(model, savepath)


##########################
def recover_closest_standard(feature_matrix_all, image_paths, save_path, n_image_samples=10, n_closest=3):
    image_paths = np.array([x[0] for x in image_paths])  # image paths

    # chose samples randomly
    sample_idxs = np.random.choice(np.arange(len(feature_matrix_all)), n_image_samples)

    faiss_search_index = faiss.IndexFlatL2(feature_matrix_all.shape[-1])  # use L2 distance
    faiss_search_index.add(feature_matrix_all)  # set database
    _, closest_feature_idxs = faiss_search_index.search(feature_matrix_all, n_closest + 1)  # search several closest neighbors
