...
def Food(opt, datapath):
	...
	image_list    = {i: sorted([image_sourcepath+'/'+key+'/'+x for x in filter(lambda x: not x.endswith(".txt"), os.listdir(image_sourcepath+'/'+key))]) for i, key in enumerate(image_classes)}
	...
	# train_idx = int(len(keys)*0.7)
	train_idx = int(len(keys)*0.7)
	train,test      = keys[:train_idx], keys[train_idx:]
