...
class Histogram(nn.Cell):
    def __init__(self, opt):
        ...
        self.support_torch  = ops.linspace(-1, 1, self.nbins).reshape(-1, 1)  # .to(opt.device)
    ...
    def construct(self, batch, labels, **kwargs):
        ...
        sim_inds = ops.triu(ops.ones(similarity.shape), 1).bool()  # .to(self.par.device)
        ...
        assigned_bin_values = ((unique_sim_rep.asnumpy() + 1) / self.bin_width).astype(int) * self.bin_width - 1
        ...
        hist_pos_rep  = hist_pos_sim.view(-1, 1).repeat(1, hist_pos_sim.shape[0])
        hist_pos_inds = ops.tril(ops.ones(hist_pos_rep.shape), -1).bool()
        hist_pos_rep[hist_pos_inds] = 0
        hist_pos_cdf  = hist_pos_rep.sum(0)

        loss = ops.sum(hist_neg_sim * hist_pos_cdf)
    ...
    def histogram(self, unique_sim_rep, assigned_bin_values, idxs, n_elem):
        ...
        indsa = Tensor((assigned_bin_values == (self.support-self.bin_width) ) & idxs.asnumpy())
        indsb = Tensor((assigned_bin_values == self.support) & idxs.asnumpy())
