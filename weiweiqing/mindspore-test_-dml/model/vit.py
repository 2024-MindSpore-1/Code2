...
class Attention(nn.Cell):
	def __init__(
		...
		self.out = nn.Dense(dim, dim)  # for last projection
...
class ViT(nn.Cell):
	def __init__(
		...
		# position embedding
		# for each patch, num_patchers + 1, +1 for cls_token
	...
	def construct(self, x):
		...
		x = self.patch_embedding(x)  # [b, n, embed_dim] b-batch_size, n-num_patches
		cls_tokens = ops.tile(self.cls_token.astype(x.dtype), (x.shape[0], 1, 1))  # [b, 1, embed_dim]
		x = ops.concat((cls_tokens, x), axis=1)  # [b, n+1, embed_dim]
		x += self.pos_embedding  # [b, n+1, embed_dim] + [1, n + 1, embed_dim] => [b, n+1, embed_dim]
		x = self.pos_dropout(x)  # x: [batch_size, num_patches+1, embed_dim] note, +1: class token

		x = self.transformer(x)  # [b, n+1, embed_dim]
		x = self.norm(x)  # LayerNorm(embed_dim)

		x = x[:, 0]  # [b, embed_dim]
		if self.training:
			x = self.dropout(x)
		x = self.dense(x)  # [b, n_classes]
