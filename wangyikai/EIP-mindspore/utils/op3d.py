...
from open3d.io import load_obj
from open3d.loss import chamfer_distance
from open3d.ops import sample_points_from_meshes
from open3d.ops.knn import knn_points
from open3d.structures import Meshes
import numpy as np


def point2mesh(x, filename, save_file_only=False):
    pcd = o3d.geometry.PointCloud()
    points = x.detach().cpu().numpy() if isinstance(x, ops.Tensor) else x
    for point in points:
    ...
    return mesh, ops.Tensor(normals)
...
def get_mesh_chamfer_distance(src_mesh, trg_mesh):
    scale = 100
    src_sample = sample_points_from_meshes(src_mesh, 5000).detach() * scale  # ops.Size([1, 5000, 3])
...
def get_knn_vector(x_boundary, trg_mesh):
    ...
    lengths = ops.tensor([x_boundary.shape[0]])
    ...
    return x_knn_vector, x_knn_vector_norm  # ops.Size([n, 3]), ops.Size([n])
