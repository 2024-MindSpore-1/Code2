...
# import torch
import mindspore
# from torch.autograd import Variable # 对应 mindspore.Parameter

# from src.cuda import CUDA # 已注释
...
def get_minibatch(lines, tok2id, index, batch_size, max_len, sort=False, idx=None,
    ...
    # 修改
    input_lines = mindspore.Parameter(mindspore.Tensor(input_lines, dtype=mindspore.int64))
    output_lines = mindspore.Parameter(mindspore.Tensor(output_lines, dtype=mindspore.int64))
    # mask = mindspore.Parameter(mindspore.Tensor(mask, dtype=mindspore.float64))
    mask = mindspore.Parameter(mindspore.Tensor(mask))
...
def minibatch(src, tgt, idx, batch_size, max_len, model_type, is_test=False):
    ...
    if model_type == 'delete':
        ...
        # 修改
        attribute_ids = mindspore.Parameter(mindspore.Tensor(attribute_ids, dtype=mindspore.int64))
